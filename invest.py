...
while True:
    ...
    for_cookies = session.get('http://www.fundamentus.com.br/index.php')
    ...
    for codigo_acao in acoes_ibov["Código"][:2]:
        ...
        r = session.get(url+codigo_acao, proxies = proxyDict,headers = header, cookies = cookies)#, headers = header)
        print(r.text)
